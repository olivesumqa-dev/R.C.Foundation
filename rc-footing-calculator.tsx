...
import { SaveDialog, LoadDialog } from "./calculator-save-load";
import {
...
export default function RCFootingCalculator({ onClose, title = "Isolated Footing Design" }: Props) {
  ...
  const doSave = async (name: string, folder: string) => {
    if (showSave === "save" && currentId !== null) {
      const r = await fetch(`/api/calculations/${currentId}`, {
        method: "PUT", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, folder, data: inputs }),
      });
      if (!r.ok) throw new Error("Save failed.");
      const rec = await r.json();
      setCurrentName(rec.name); setCurrentFolder(rec.folder);
      flash(`Saved "${rec.name}"`);
    } else {
      const r = await fetch(`/api/calculations`, {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ calcType: CALC_TYPE, name, folder, data: inputs }),
      });
      if (!r.ok) throw new Error("Save failed.");
      const rec = await r.json();
  ...
  const doLoad = async (id: number) => {
    const r = await fetch(`/api/calculations/${id}`, { credentials: "include" });
